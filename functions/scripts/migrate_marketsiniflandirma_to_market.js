...
const SRC_COLLECTION = 'MarketSiniflandirma';
const DST_COLLECTION = 'market';
const BATCH = Number(process.env.MIGRATE_BATCH || 150);
const STATE_FILE = path.join(__dirname, 'migrate_marketsiniflandirma_to_market.state.json');
...
async function main() {
  ...
  const srcApp = admin.initializeApp({ credential: admin.credential.cert(srcCred) }, 'src-market-mig');
  const dstApp = admin.initializeApp({ credential: admin.credential.cert(dstCred) }, 'dst-market-mig');
