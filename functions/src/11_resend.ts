...
function parsePurpose(raw: unknown): VerificationPurpose {
  const value = String(raw || "signup").trim() as VerificationPurpose;
...
async function resolveCaller(request: CallableRequest): Promise<{ uid: string; email: string }> {
  ...
  if (!idToken) {
    throw new HttpsError("unauthenticated", "Kullanıcı doğrulanmamış");
  }
  ...
  try {
    ...
    throw new HttpsError("unauthenticated", "Geçersiz oturum tokenı");
  }
}

function buildMailCopy(purpose: VerificationPurpose): { subject: string; intro: string } {
...
function buildMailHtml(purpose: VerificationPurpose, code: string): string {
...
async function ensurePurposeEmailRules(
  ...
  const ensureEmailBelongsToCaller = async () => {
    ...
    if (!profileEmail || profileEmail !== emailLower) {
      throw new HttpsError(
        "permission-denied",
        "Bu işlem için hesabınızdaki kayıtlı e-posta adresini kullanmalısınız",
      );
    }
  };
  ...
  if (purpose === "email_change") {
    ...
    try {
      ...
      throw new HttpsError("internal", "E-posta kontrolü sırasında hata oluştu");
    }
    return;
  }
...
async function sendCodeInternal(
  ...
  if (existing.exists) {
    ...
    if (dailyKey === dayKey && dailyCount >= EMAIL_DAILY_LIMIT) {
      throw new HttpsError("resource-exhausted", "Günlük e-posta kod limiti aşıldı");
    }
  }

  const apiKey = RESEND_API_KEY.value() || process.env.RESEND_API_KEY || "";
  if (!apiKey) {
    throw new HttpsError("failed-precondition", "Resend API key tanımlı değil");
  }
...
export const markCurrentEmailVerified = onCall(
  {
    region: REGION,
    timeoutSeconds: 60,
    memory: "256MiB",
  ...
  async (request: CallableRequest) => {
    ...
    if (!emailToConfirm) {
      throw new HttpsError("failed-precondition", "Hesabınızda e-posta bulunamadı");
    }

    const codeRef = verificationRef(emailToConfirm, "email_confirm");
    const verificationSnap = await codeRef.get();
    if (!verificationSnap.exists) {
      throw new HttpsError("failed-precondition", "E-posta onayı gerekli");
    }
    ...
    try {
      ...
      if (
        ...
        throw new HttpsError("already-exists", "Bu e-posta başka bir hesapta kullanılıyor");
      } else if (code === "auth/invalid-email") {
        throw new HttpsError("invalid-argument", "E-posta formatı geçersiz");
      } else if (code === "auth/user-not-found") {
        throw new HttpsError("not-found", "Kullanıcı bulunamadı");
      } else {
        throw new HttpsError(
          "internal",
          message.length > 0 ? `Auth güncelleme hatası: ${message}` : "Auth e-posta onayı güncellenemedi",
        );
      }
    }
...
export const updateUserPhoneNumberAfterEmailVerification = onCall(
  {
    region: REGION,
    timeoutSeconds: 60,
    memory: "256MiB",
  ...
  async (request: CallableRequest) => {
    ...
    const verificationSnap = await codeRef.get();
    if (!verificationSnap.exists) {
      throw new HttpsError("failed-precondition", "Telefon değişikliği için e-posta doğrulaması gerekli");
    }
