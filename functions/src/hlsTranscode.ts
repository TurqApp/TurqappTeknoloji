...
const clampEvenDimension = (value: unknown, fallback: number): number => {
  const n = Number(value);
...
interface VideoTarget {
  ...
  buildSuccessData: (hlsUrl: string, hlsSegmentCount: number) => Record<string, unknown>;
...
export const onVideoUpload = functions
  ...
  .onFinalize(async (object) => {
    ...
    try {
      ...
      const segmentConfigPath =
        target.type === "story" ? "adminConfig/storyHlsSegment" : "adminConfig/hlsSegment";
      ...
      const targetShortSide = clampEvenDimension(
        Math.min(sourceShortSide || 720, 1080),
        720
      );
      const singleBitrate =
        targetShortSide >= 1080 ? 5000 :
      ...
      console.log(
        `[HLS] Single rendition mode enabled (${targetShortSide} short-side, ${singleBitrate}k, hasAudio=${videoHasAudio})`
      );
      ...
      if (!migrationMode) {
        await db.doc(target.firestoreDoc).set(
          target.buildSuccessData(hlsUrl, hlsSegmentCount),
          { merge: true }
        );
      }
