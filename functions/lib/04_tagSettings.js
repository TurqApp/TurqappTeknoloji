...
exports.getTagSettings = getTagSettings;
exports.generatePostTags = generatePostTags;
exports.generateTagDetails = generateTagDetails;
exports.writeTagIndex = writeTagIndex;
...
function isValidUserTag(w, cfg) {
    if (!w)
        return false;
    if (w.length < cfg.tagMinLength || w.length > cfg.tagMaxLength)
        return false;
    if (cfg.bannedWords.includes(normalizeTR(w)))
        return false;
    if (/^\d+$/.test(w))
        return false;
    return true;
}
function stripSuffixes(word, suffixes) {
...
function isValidTag(w, cfg) {
    ...
    if (cfg.bannedWords.includes(normalizeTR(w)))
        return false;
    if (cfg.stopwords.includes(normalizeTR(w)))
        return false;
    if (/^\d+$/.test(w))
        return false;
    return true;
}
function startsWithUppercaseTR(word) {
    if (!word)
        return false;
    const first = word[0];
...
async function getTagSettings() {
    ...
    const suffixesRaw = Array.isArray(d.suffixes)
        ? d.suffixes.map((x) => normalizeTR(String(x)))
        : DEFAULT_TAG_SETTINGS.suffixes;
    const suffixes = suffixesRaw.filter((s) => s.length >= 2).slice(0, 4);
    ...
    return {
        stopwords: Array.isArray(d.stopwords) ? d.stopwords.map((x) => normalizeTR(String(x))) : DEFAULT_TAG_SETTINGS.stopwords,
        suffixes,
        bannedWords: Array.isArray(d.bannedWords) ? d.bannedWords.map((x) => normalizeTR(String(x))) : DEFAULT_TAG_SETTINGS.bannedWords,
        tagMinLength,
        tagMaxLength,
...
async function generatePostTags(input) {
    ...
    if (cfg.enableBigrams) {
        ...
        const allow = new Set((cfg.bigram.allowList || []).map((p) => `${normalizeTR(p[0])}${joinChar}${normalizeTR(p[1])}`));
        const deny = new Set((cfg.bigram.denyList || []).map((p) => `${normalizeTR(p[0])}${joinChar}${normalizeTR(p[1])}`));
        for (const [bi, count] of counts.entries()) {
            if (allow.size > 0 && !allow.has(bi))
                continue;
            if (deny.has(bi))
                continue;
            if (count < Math.max(1, cfg.bigram.minScore || 1))
                continue;
...
function generateTagsFromText(text, cfg, maxTags, exclude) {
    const score = new Map();
    const wordsRaw = splitWords(text);
    for (const w0 of wordsRaw) {
        if (w0.startsWith("#"))
...
async function writeTagIndex(postId, tags, meta) {
    ensureAdmin();
    const db = (0, firestore_1.getFirestore)();
        ...
        await db.runTransaction(async (tx) => {
            ...
            for (let idx = 0; idx < chunk.length; idx++) {
                ...
                if (!tagPostSnap.exists) {
                    ...
                    tx.set(hasHashtag ? postHashtagRef : postTagRef, {
                        tag,
                        postId,
                        hasHashtag,
                        updatedAt: now,
...
//# sourceMappingURL=04_tagSettings.js.map