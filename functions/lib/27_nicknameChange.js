...
exports.changeOwnNickname = void 0;
...
function extractGraceWindowStartAt(data) {
    if (!data)
        return null;
    return parseMillis(data.nicknameGraceWindowStartAt);
}
function historyEntry(fromNickname, toNickname, changedAt) {
    return {
...
exports.changeOwnNickname = functions
    .region("europe-west3")
    ...
    const normalized = (0, userSchemaUtils_1.normalizeUsernameLower)(data?.nickname);
    if (!normalized) {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        const currentNickname = (0, userSchemaUtils_1.normalizeUsernameLower)(userData.usernameLower || userData.username || userData.nickname);
        ...
        if (currentNickname && currentNickname !== normalized) {
            ...
            patch.nicknameHistory = admin.firestore.FieldValue.arrayUnion(historyEntry(currentNickname, normalized, nowMs));
        }
...
//# sourceMappingURL=27_nicknameChange.js.map