...
import 'UserStoryContent.dart';
import '../StoryMaker/StoryMakerController.dart';
import 'package:turqappv2/Core/ConnectivityHelper.dart';
...
class _StoryViewerState extends State<StoryViewer>
  ...
  void _prefetchNext(int index) async {
    // Sonraki kullanıcının ilk görsel/gif öğesini önbelleğe al
    try {
      // Sadece Wi‑Fi bağlantısında prefetch yap
      final isWifi = await ConnectivityHelper.isWifi();
      if (!isWifi) return;
      final next = index + 1;
      if (next >= widget.storyOwnerUsers.length) return;
      final nextUser = widget.storyOwnerUsers[next];
      if (nextUser.stories.isEmpty) return;
      final firstStory = nextUser.stories.first;
      final firstImage = firstStory.elements.firstWhere(
        (e) =>
            e.type == StoryElementType.image || e.type == StoryElementType.gif,
    ...
    try {
      ...
      final lastSeen =
          StoryInteractionOptimizer.to.localTimeCache[user.userID] ??
              Get.find<FirebaseMyStore>().readStoriesTimes.value[user.userID];
