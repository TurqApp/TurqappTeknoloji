...
extension AntremanScoreControllerDataPart on AntremanScoreController {
  bool _applyWarmCache() {
    final cached = AntremanScoreController._cachedLeaderboard;
    final cachedAt = AntremanScoreController._cachedAt;
    if (cached == null || cachedAt == null) return false;
    if (AntremanScoreController._cachedMonthKey != _monthKey) return false;
    if (DateTime.now().difference(cachedAt) >
        AntremanScoreController._cacheTtl) {
      return false;
    }
  ...
  bool _isEligibleEntry(Map<String, dynamic> data) {
    ...
    if (AntremanScoreController._excludedRozet.contains(rozet)) return false;
    final nickname =
  ...
  Future<void> fetchLeaderboard({bool showLoader = true}) async {
    try {
      ...
      AntremanScoreController._cachedLeaderboard =
          List<Map<String, dynamic>>.from(limited);
      AntremanScoreController._cachedAt = DateTime.now();
      AntremanScoreController._cachedMonthKey = _monthKey;
