...
class PostEditingService extends GetxController {
  ...
  static const List<String> _commonHashtags = [
    '#fotoğraf', '#video', '#müzik', '#sanat', '#seyahat', '#yemek',
    '#spor', '#teknoloji', '#kitap', '#film', '#doğa', '#arkadaş'
  ];
  ...
  List<SmartSuggestion> _generateHashtagSuggestions(String text) {
    ...
    for (final hashtag in _commonHashtags) {
      ...
      if (words.any((word) => word.contains(keyword)) && !text.contains(hashtag)) {
  ...
  Map<String, dynamic> getEditStatistics() {
    ...
    final recentActions = _undoStack.where((action) =>
        now.difference(action.timestamp).inMinutes < 30).length;
