...
class OptimizedNSFWService {
  ...
  static Future<NSFWCheckResult> checkImage(File imageFile) async {
    ...
    try {
      ...
      final prepared = await _prepareImageForDetection(imageFile);
      tempFileToDelete = prepared.tempFileToDelete;

      // Perform NSFW check on normalized image when available
      final result = await _detector!.detectNSFWFromFile(prepared.fileForCheck);
  ...
  static Future<_PreparedImageForCheck> _prepareImageForDetection(
    File imageFile,
  ) async {
    try {
      ...
      if (decoded == null) {
        return _PreparedImageForCheck(
          fileForCheck: imageFile,
      ...
      final normalizedPath = p.join(
        tempDir.path,
        'nsfw_img_${DateTime.now().microsecondsSinceEpoch}.jpg',
      );
      final normalizedFile = File(normalizedPath);
      await normalizedFile.writeAsBytes(jpgBytes, flush: true);

      return _PreparedImageForCheck(
        fileForCheck: normalizedFile,
        tempFileToDelete: normalizedFile,
      );
    } catch (_) {
      return _PreparedImageForCheck(
        fileForCheck: imageFile,
...
class _PreparedImageForCheck {
  final File fileForCheck;
  final File? tempFileToDelete;

  const _PreparedImageForCheck({
    required this.fileForCheck,
    required this.tempFileToDelete,
  });
}
