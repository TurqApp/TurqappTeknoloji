...
| Market | `Market`, `detail`, `search`, `saved`, `offers`, `my items`, `create`, `filter` | manuel | `MANUEL_YESIL` | ilk acilis stabil; `Ilan Detayi -> liste` geri donusu temiz ve owner `Tekliflerim` ekrani veriyle acildi. `saved/my items/create/filter` icin ikinci sweep gerekli |
...
| Practice exams | `Online Sinav`, `saved`, `my exams`, `type list`, `sonuclarim` | manuel + omurga sweep | `ANDROID_ACIK` | `Online Sinav` liste, detail modal ve owner CTA Android cihazda temiz dogrulandi; owner kartlarda yanlis `Hemen Basvur` etiketi `Goruntule` olarak duzeltildi. Ayrı applicant hesapla gercek `Basvur` akisi henuz dogrulanmadi |
...
| Other education | `Scholarships`, `PreviousQuestions`, `CikmisSorular`, `Tests`, `Antreman3`, `QuestionBank` | parcali | `ANDROID_ACIK` | `Burslar` liste ve `Burs Detayi` Android cihazda temiz acildi. `Soru Bankasi` icin LGS/YKS vb. kartlara test hook eklendi ve `LGS` karti Android cihazda acildi. Kalan acik: arama barı focus'u klavyeyi agresif tutuyor; bu davranis UX sweep'inde ayrıca temizlenmeli |
