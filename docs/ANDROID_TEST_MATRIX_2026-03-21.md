...
| Other education | `Scholarships`, `PreviousQuestions`, `CikmisSorular`, `Tests`, `Antreman3`, `QuestionBank` | parcali | `ANDROID_ACIK` | `Burslar` liste ve `Burs Detayi` Android cihazda temiz acildi. `Soru Bankasi` ana liste acildi, ancak arama barı focus'u klavyeyi agresif tutuyor; kategori/detail sweep'i icin test hook veya focus duzeltmesi gerekiyor |
