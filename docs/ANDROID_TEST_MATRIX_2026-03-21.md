...
| Practice exams | `Online Sinav`, `saved`, `my exams`, `type list`, `sonuclarim` | manuel + omurga sweep | `KISMEN` | `Online Sinav` liste, detail modal ve owner CTA Android cihazda temiz dogrulandi; `Denemeler` tip listesinde `LGS` karti acilip `LGS Testleri` ekrani dogrulandi; action menu uzerinden `Sonuclarim` ve `Kaydedilenler` ekranlari Android cihazda acildi. Ayrı applicant hesapla gercek `Basvur` akisi hala acik |
