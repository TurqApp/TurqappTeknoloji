...
| Market | `Market`, `detail`, `search`, `saved`, `offers`, `my items`, `create`, `filter` | manuel | `MANUEL_YESIL` | ilk acilis stabil; `Ilan Detayi -> liste` geri donusu temiz, owner `Tekliflerim` ekrani veriyle acildi, `Kaydettiklerim` ve `Ilanlarim` yuzeyleri Android cihazda veriyle dogrulandi. `create/filter` icin ikinci sweep gerekli |
