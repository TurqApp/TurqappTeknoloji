...
- `integration_test/` kritik smoke test dizini
  Durum: `KISMEN`
  Not: dizin + ortak bootstrap helper + 5 kritik smoke dosyasi acildi. Ilk dilim artik kararlı test key'leri ile `Feed`, `Explore`, `Profile`, `Short`, `Notifications` ekran hedeflerini buluyor; state probe ile `feed/short/profile/socialProfile/notifications/navBar` controller snapshot'i testten okunuyor; deterministic integration test mode ile startup intro/watchdog/periyodik yan etkiler sakinlestirildi; route replay helper'lari ile `Feed -> Explore/Profile/Short/Notifications -> Feed` zinciri ortaklastirildi; replay sonrasi `count zero drop`, `active doc preserve` ve `notifications unread non-negative` assertion'lari eklendi; fixture contract katmani ile `dart-define` uzerinden `minCount/docIds/maxUnread` beklentileri verilebilir hale geldi; `integration_test/fixtures/smoke_fixture.device_baseline.json` varsayilan fixture dosyasi eklendi ve `scripts/run_integration_smoke.sh` bu dosyayi ortam degiskeni verilmezse otomatik kullanacak sekilde guncellendi; her smoke senaryosu artik `artifacts/integration_smoke/*.json` altina route/state/telemetry dump'i yaziyor, fail aninda best-effort screenshot deniyor ve smoke script `artifacts/integration_smoke_report_latest.json` toplu ozetini de uretiyor. Sonraki adim fixture verisini cihaz/senaryo bazli gercek doc beklentileriyle sertlestirmek ve CI/device smoke adimina tasimak.
...
- artifact toplama: screenshot + KPI dump + route dump
  Durum: `KISMEN`
  Not: `RuntimeHealthExporter` ve smoke artifact collector ile `artifacts/integration_smoke/*.json` altina route/state probe + KPI summary + threshold report + invariant dump'i yaziliyor. Smoke fail aninda integration test binding destek verirse ayni klasore best-effort `.png` screenshot da yaziliyor; `tool/integration_smoke_report.dart` ile bunlardan toplu ozet raporu uretiliyor. Sonraki adim bunu cihaz/simulatör native screenshot ve zorunlu gate davranisiyla birlestirmek.
...
- Son tamamlanan Android market sonucu: `Mabil Pazar` liste acilisi, `Ilan Detayi -> liste` geri donusu, owner `Tekliflerim`, `Kaydettiklerim` ve `Ilanlarim` yuzeyleri Android cihazda temiz dogrulandi.
