...
policy_path="${REPO_SURFACE_BUDGET_POLICY:-config/quality/repo_surface_budget_targets.txt}"
artifact_dir="${REPO_SURFACE_BUDGET_ARTIFACT_DIR:-artifacts/repo_surface_budget}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage:" >&2
      echo "  bash scripts/check_repo_surface_budget.sh [--policy <relative-or-absolute-policy-path>]" >&2
      exit 64
      ;;
  esac
done
...
report_path="$artifact_dir/repo_surface_budget_report.txt"
inventory_path="$artifact_dir/repo_surface_budget_inventory.txt"
...
def tracked_dart_files(root_dir: Path) -> list[str]:
    output = subprocess.check_output(
        ["git", "-C", str(root_dir), "ls-files", "lib/**/*.dart"],
        text=True,
    )
...
def main() -> int:
    ...
    files = tracked_dart_files(root_dir)
    ...
    report = [
        f"Repo surface budget report generated at: {subprocess.check_output(['date', '-u', '+%Y-%m-%dT%H:%M:%SZ'], text=True).strip()}",
        f"Repository root: {root_dir}",
        f"Policy path: {policy_path}",
        f"Tracked lib Dart file count: {len(files)}",
    ...
    print(f"Repo surface budget report generated at: {report[0].split(': ', 1)[1]}")
    print(f"Policy path: {policy_path}")
    print(f"Tracked lib Dart file count: {len(files)}")
    print(f"Report path: {report_path}")
    print(f"Inventory path: {inventory_path}")

    if guard_failures:
        print()
        print("[FAIL] Repo surface budget guard exceeded guarded budgets.")
        return 3

    print()
    print("[PASS] Repo surface budget guard passed.")
    if watch_exceeded:
