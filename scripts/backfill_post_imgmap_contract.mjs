...
function canonicalizeKnownPublicPostAssetUrl(rawUrl, docId = "") {
  ...
  if (!objectPath || !objectPath.startsWith("Posts/")) {
    return text;
  }

  const relativeFromPosts = objectPath.slice("Posts/".length);
...
function buildPatch(docId, data) {
  const currentImg = Array.isArray(data.img) ? data.img.map((value) => asString(value)).filter(Boolean) : [];
  const currentImgMap = Array.isArray(data.imgMap) ? data.imgMap : [];
  if (currentImgMap.length === 0) {
    return { patch: {}, changed: {} };
  }

  let mutated = false;
  const nextImgMap = currentImgMap.map((entry, index) => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) return entry;
    const currentUrl = asString(entry.url);
    const imgUrl = asString(currentImg[index]);
    const nextUrl = imgUrl
      ? canonicalizeKnownPublicPostAssetUrl(imgUrl, docId)
      : canonicalizeKnownPublicPostAssetUrl(currentUrl, docId);
...
async function run() {
  ...
  while (true) {
    ...
    for (const doc of snap.docs) {
      ...
      const { patch, changed } = buildPatch(docId, data);
      if (Object.keys(changed).length === 0) {
        summary.unchangedPosts += 1;
      } else {
        summary.touchedPosts += 1;
        summary.patchFields.imgMap += 1;
