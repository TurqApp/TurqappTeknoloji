...
void main() {
  ...
  testWidgets(
    'OS permission matrix surfaces granted camera, photos, and microphone states',
    (tester) async {
      await SmokeArtifactCollector.runScenario(
        'permission_os_granted_state_smoke',
        tester,
        () async {
            ...
            (surface) {
              ...
              return statuses is Map<String, dynamic> &&
                  (statuses['camera'] == 'granted' ||
                      statuses['camera'] == 'limited') &&
                  (statuses['photos'] == 'granted' ||
                      statuses['photos'] == 'limited') &&
                  (statuses['microphone'] == 'granted' ||
                      statuses['microphone'] == 'limited');
            },
            maxPumps: 24,
            reason: 'OS-level granted permissions did not propagate to probe.',
          );

          final statuses =
              Map<String, dynamic>.from(payload['statuses'] as Map);
          expect(statuses['camera'], anyOf('granted', 'limited'));
          expect(statuses['photos'], anyOf('granted', 'limited'));
          expect(statuses['microphone'], anyOf('granted', 'limited'));
        },
      );
    },
